...
// ── Types ─────────────────────────────────────────────────────────────────────
// Sesuaikan field dengan kolom aktual di tabel putusan_pajak kamu
...
function amarStyle(amar: string) {
  ...
  if (l.includes("tolak") || l.includes("menolak"))
    return { badge: "bg-[#ffacac] text-red-600 border-red-300", big: "text-red-600" };
  if (l.includes("kabul") || l.includes("menerima"))
    return { badge: "bg-emerald-100 text-emerald-700 border-emerald-300", big: "text-emerald-600" };
  if (l.includes("batal"))
    return { badge: "bg-purple-100 text-purple-700 border-purple-300", big: "text-purple-600" };
  return { badge: "bg-amber-100 text-amber-700 border-amber-300", big: "text-amber-600" };
}
...
function PartyCard({ role, name, color }: { role: string; name?: string; color: string }) {
  return (
          ...
          <div className="flex flex-wrap gap-3">
            {d.hakim_ketua && (
              <div className="flex items-center gap-3 bg-white rounded-full border border-[var(--pajak-border)] px-4 py-2.5 shadow-sm">
                ...
                <div>
                  <p className="text-[9px] uppercase text-gray-400 font-semibold" style={{ fontFamily: "var(--font-montserrat)" }}>Hakim Ketua</p>
                  <p className="text-[12px] text-black" style={{ fontFamily: "var(--font-coolvetica)" }}>{d.hakim_ketua}</p>
                </div>
              </div>
            )}
            {hakimAnggota.map((h, i) => (
              <div key={i} className="flex items-center gap-3 bg-white rounded-full border border-[var(--pajak-border)] px-4 py-2.5 shadow-sm">
                <div className="w-7 h-7 rounded-full bg-emerald-100 flex items-center justify-center">
                  <Users size={12} className="text-emerald-600" />
                </div>
                <div>
                  <p className="text-[9px] uppercase text-gray-400 font-semibold" style={{ fontFamily: "var(--font-montserrat)" }}>Hakim Anggota</p>
                  <p className="text-[12px] text-black" style={{ fontFamily: "var(--font-coolvetica)" }}>{h}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-[var(--pajak-border)] p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-[10px] font-semibold uppercase text-gray-400 tracking-wide" style={{ fontFamily: "var(--font-montserrat)" }}>
              Pemohon Banding
            </p>
          </div>
          <p className="text-sm text-gray-800 leading-relaxed" style={{ fontFamily: "var(--font-montserrat)" }}>
            {d.argumen_pemohon || "-"}
          </p>
        </div>
        <div className="bg-white rounded-2xl border border-[var(--pajak-border)] p-5 shadow-sm">
...
function PertimbanganTab({ d }: { d: PutusanRow }) {
  return <Card label="Pertimbangan Majelis Hakim" value={d.pertimbangan_hakim} />;
}
  ...
  return (
    <div className="space-y-4">
      <div
        ...
        style={{
          borderColor:
            d.amar_putusan?.toLowerCase().includes("tolak") ? "#ffacac"
            : d.amar_putusan?.toLowerCase().includes("kabul") ? "#6ee7b7"
            : "#fcd34d",
        }}
      >
        <p
        ...
        {data && !loading && (
          <>
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
              <div>
                ...
                <div className="flex items-center gap-3 flex-wrap mt-3">
                  ...
                  {data.tanggal_putusan && (
                    <span className="flex items-center gap-1 text-xs text-gray-400" style={{ fontFamily: "var(--font-montserrat)" }}>
                      <Calendar size={11} />
              ...
              {/* PDF button */}
              <a
                href={`/pdf/${encodeURIComponent(data.nomor_putusan_pp)}`}
                target="_blank"
                className="flex items-center gap-1.5 bg-[var(--pajak-primary)] text-white text-xs px-4 py-2.5 rounded-lg hover:brightness-110 transition-all shadow-sm shrink-0"
                style={{ fontFamily: "var(--font-coolvetica)" }}
              >
                <FileText size={12} />
                Lihat PDF
            ...
            >
              {data.pemohon && (
                <>
                  <span className="font-semibold uppercase tracking-wide text-gray-400">Pemohon:</span>
              ...
              {data.terbanding && (
                <>
                  <span className="font-semibold uppercase tracking-wide text-gray-400">Terbanding:</span>
              ...
              {data.tahun_pajak && (
                <>
                  <span className="font-semibold uppercase tracking-wide text-gray-400">Tahun Pajak:</span>
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              {data.jenis_pajak && (
                <InfoPill icon={<Coins size={13} />} label="Jenis Pajak" value={data.jenis_pajak} color="#0C81E4" />
              )}
              {data.upaya_hukum && (
                <InfoPill icon={<BookOpen size={13} />} label="Upaya Hukum" value={data.upaya_hukum} color="#11C4D4" />
              )}
              {data.pengadilan && (
                <InfoPill icon={<Landmark size={13} />} label="Pengadilan" value={data.pengadilan} color="#4FE7AF" />
              )}
              {data.nilai_sengketa && (
                <InfoPill icon={<Scale size={13} />} label="Nilai Sengketa" value={formatCurrency(data.nilai_sengketa)} color="#FFB700" />
              )}
            </div>
              ...
              {TABS.map((tab) => (
                <button
                  ...
                  className={`pb-3 text-base whitespace-nowrap transition-colors ${
                    activeTab === tab.key
                      ? "text-[var(--pajak-primary)] border-b-2 border-[var(--pajak-primary)] -mb-px"
